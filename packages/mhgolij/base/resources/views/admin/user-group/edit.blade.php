            ...
            <ol class="inline-flex items-center">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span
                            class="mx-1 text-sm font-medium text-gray-500 md:mx-2 dark:text-gray-400">@lang('mhgolijBase::admin.breadcrumb.edit_user_group',['name'=>$userGroup->name])</span>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="rounded shadow-2xl bg-gray-800 p-5 m-5 text-white">
            <h2>@lang('mhgolijBase::admin.breadcrumb.edit_user_group',['name'=>$userGroup->name])</h2>
        </div>
        <form action="{{route('base.admin.user-group.update',$userGroup->code)}}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid gap-4 overflow-y-auto grid-cols-1 md:grid-cols-4 m-5 rounded shadow-2xl bg-gray-800 p-5">
                ...
                <div>
                    ...
                    <select type="text" id="is_admin" name="is_admin"
                        ...
                        <option @selected(old('is_admin',$userGroup->is_admin) == 0) value="0">@lang('mhgolijBase::admin.message.no')</option>
                        <option @selected(old('is_admin',$userGroup->is_admin) == 1) value="1">@lang('mhgolijBase::admin.message.yes')</option>
                    </select>
                    @error('is_admin')
                    <span class="text-red-600 pr-2 text-sm">{{$message}}</span>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="limitation_count" name="limitation_count" value="{{old('limitation_count',$userGroup->limitation_count)}}"
                        @class([
                        "bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"=>true,
                ...
                <div>
                    <span class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">@lang('mhgolijBase::admin.form.code')</span>
                    <span type="text" dir="ltr"
                        class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    >{{$userGroup->code}}</span>
                </div>
