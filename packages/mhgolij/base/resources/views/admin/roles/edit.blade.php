            ...
            <ol class="inline-flex items-center">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span
                            class="mx-1 text-sm font-medium text-gray-500 md:mx-2 dark:text-gray-400">@lang('mhgolijBase::admin.breadcrumb.edit_role',['name'=>$role->fa_name])</span>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="rounded shadow-2xl bg-gray-800 p-5 m-5 text-white">
            <h2>@lang('mhgolijBase::admin.breadcrumb.edit_role',['name'=>$role->fa_name])</h2>
        </div>
        <form action="{{route('base.admin.roles.update',$role->name)}}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid gap-4 overflow-y-auto grid-cols-1 md:grid-cols-4 m-5 rounded shadow-2xl bg-gray-800 p-5">
                <div>
                    ...
                    <input type="text" id="fa_name" name="fa_name" value="{{old('fa_name',$role->fa_name)}}"
                        @class([
                             ...
                             "border-red-600"=> $errors->has('fa_name'),
                             "border-gray-300 dark:border-gray-600"=> !$errors->has('fa_name'),
                         ])/>
                    @error('fa_name')
                ...
                @foreach($permissions as $permission)
                    <div class="flex items-center gap-2">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" value="{{$permission->name}}" name="permission[]" @checked(in_array($permission->name,old('permission',$rolePermission)??[])) class="sr-only peer">
                            <div
